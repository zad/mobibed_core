 ...
 *
 */
public class MobiBedSocketDemo extends drcl.net.Module 
	...
	protected int MAX_PKT_LEN = 1024*2;
	protected int INIT_PKT_BUF_NUM = 50;
	protected BufferArray rbuffer = new BufferArray(50,MAX_PKT_LEN);
	protected BufferArray sbuffer = new BufferArray(50,MAX_PKT_LEN);
	
	private NonBlockingQueue buffer;
	protected long localAddr; 
	
	public MobiBedSocketDemo(){
		super();
		init();
	}
	...
	private void init(){
		...
		buffer = new NonBlockingQueue(INIT_PKT_BUF_NUM);
		// set default port
		localPort = 5003;
	}
...
//	}
	...
	public MobiBedSocketDemo(String id_){
		super(id_);
	}
...
//			System.out.println("delay2--------------------"+delay);
	...
	protected void receiving(){
		if(isDebugEnabled())
			debug("JavaSocket is listening on Port " + localPort);
		try{
			while(SOCKET_ON)
			{
				InetPacket pkt = buffer.poll();
